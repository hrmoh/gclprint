...
CEncrypt::CEncrypt(CString keyString)
{
	...
	//initialisationSuccess=CryptAcquireContext(&cspContext,NULL,NULL,PROV_RSA_FULL ,0);
	 initialisationSuccess=CryptAcquireContext(&cspContext,NULL,MS_DEF_PROV,PROV_RSA_FULL ,CRYPT_VERIFYCONTEXT);
	...
	if(initialisationSuccess)
	{
		initialisationSuccess=CryptHashData(hashData,(const unsigned char *)encryptKey.GetBuffer(256),encryptKey.GetLength(),0);
	}
